...
class GAT(nn.Module):
    def __init__(
        ...
        self.inserted_fcs1 = nn.ModuleList()
        self.inserted_fcs2 = nn.ModuleList()
        ...
        for i in range(n_layers):
            ...
            self.inserted_fcs1.append(nn.Linear(n_heads * out_hidden, n_heads * out_hidden))
            self.inserted_fcs2.append(nn.Linear(n_heads * out_hidden, n_heads * out_hidden))
    ...
    def forward(self, g):
        ...
        for i in range(self.n_layers):
            ...
            h = F.relu(self.inserted_fcs1[i](h))
            h = F.relu(self.inserted_fcs2[i](h))
            h = self.norms[i](h)
