...
class GATConv(nn.Module):
    def __init__(
        ...
        self.set_allow_zero_in_degree(True)
...
class GAT(nn.Module):
    def __init__(
        ...
        if edge_emb > 0:
            self.edge_encoder1 = nn.ModuleList()
            self.edge_encoder2 = nn.ModuleList()
        else:
            self.edge_encoder1 = None
            self.edge_encoder2 = None
        ...
        for i in range(n_layers):
            ...
            if edge_emb > 0:
                self.edge_encoder1.append(nn.Linear(edge_feats, edge_emb))
    ...
    def forward(self, g):
        ...
        for i in range(self.n_layers):
            if self.edge_encoder1 is not None:
                efeat1 = subgraphs[i].edges['to'].data["feat"]
                efeat_emb1 = self.edge_encoder1[i](efeat1)
                efeat_emb1 = F.relu(efeat_emb1, inplace=True)

                efeat2 = subgraphs[i].edges['_to'].data["feat"]
                efeat_emb2 = self.edge_encoder2[i](efeat2)
                efeat_emb2 = F.relu(efeat_emb2, inplace=True)
            else:
                efeat_emb1 = None
                efeat_emb2 = None

            h1 = self.convs1[i](subgraphs[i]['to'], h, efeat_emb1).flatten(1, -1)
            h2 = self.convs2[i](subgraphs[i]['_to'], h, efeat_emb2).flatten(1, -1)
            h = h1 + h2
